...
const Cart = ({ isOpen, toggleCart }) => {
  ...
  // Set interval to remove expired items every minute
  useEffect(() => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full bg-[#232323] z-[1000] shadow-lg transition-transform transform ${
        isOpen ? "translate-x-0" : "translate-x-full"
      } w-80 text-white`}
    >
      <div className="flex justify-between items-center p-4 bg-opacity-90 bg-[#131313] ">
        <h2 className="text-2xl font-semibold">Your Cart</h2>
        <button
          onClick={toggleCart}
          className="bg-[#232323] text-white py-1 px-3 rounded shadow hover:bg-stone-600 transition"
        >
          Close
        </button>
      </div>

      <ul className="p-4 space-y-4 h-[72%] overflow-y-auto">
        {cartItems.length > 0 ? (
          cartItems.map((item) => (
            <li
              key={item.id}
            ...
            >
              <div className="flex items-start space-x-4">
                <img src={item.image} className="w-20 h-20" alt={item.name} />
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center mt-2  text-xl text-[#e9e61b] ">
                    <button
                      onClick={() => handleDecrease(item.id)}
                      className="w-8 h-8 flex items-center justify-center bg-stone-600  rounded-full hover:bg-stone-500 transition-colors"
                    >
                      -
                    </button>
              ...
              <button
                ...
                // className="ml-4 self-end   stroke-yellow-300 fill-yellow-300   hover:stroke-red-600 hover:fill-red-600 "
                className="ml-4 self-end  text-red-600 hover:text-red-400 "
              >
          ...
          <div className="h-96 gap-y-6 flex flex-col items-center justify-center">
      ...
      {cartItems.length > 0 && (
        <div className="p-4 border-t border-neutral-500 bg-[#131313] bg-opacity-90">
          <p className="text-lg font-bold">Total Price: ${totalPrice}</p>
          <button
            onClick={toggleCart}
            className="w-full mt-4 bg-green-500 py-2 rounded shadow hover:bg-green-400 transition"
          >
            Proceed to Checkout
          </button>
        </div>
